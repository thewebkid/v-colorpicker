...
/**
 * Generate hue gradient with configurable granularity
 * @param {number} gran - granularity (degrees between stops)
 * @param {Object} [hsx] - optional HSx values to use for s/l/v
 */
export const hueGradient = (gran = 3, hsx = null) => {
  let h = 0;
  let s = 100;
  let l = 50;
  let v = null;
  ...
  while (h < 360) {
    const color = Color.parse(isHsv ? { h, s, v } : { h, s, l });
    const pos = h / 360;
  ...
  h = 359;
  const color = Color.parse(isHsv ? { h, s, v } : { h, s, l });
  stops.push(cs(color, 1));

  return stops.join(', ');
};

/**
 * Generate channel gradient for preview bars
 * @param {Color} c - base color
 * @param {string} channel - channel name (r,g,b,h,s,l,v,a)
 */
